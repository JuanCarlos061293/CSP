...
public class FitnessL extends Thread {
  private Integer fL;
   ...
   *                           thread group
   */
  public FitnessL(ThreadGroup group, String name, List<Ratio> ratios, List<Item> individuo,
   ...
   * @param name the name of the new thread
   */
  public FitnessL(String name, List<Ratio> ratios, List<Item> individuo,
                  List<Vehicle> vehiclesSec, List<Vehicle> vehiclesNoSec, List<GrupoClase> clases) {
    super(name);
    this.fL = null;
    this.ratios = ratios;
   ...
   */
  public FitnessL(List<Ratio> ratios, List<Item> individuo, List<Vehicle> vehiclesSec,
  ...
  @Override
  public void run() {
    ThreadGroup group = new ThreadGroup(("L"));
  ...
  private List<Byte> generarVectorOpcion(int opcion, int ventana) {
    ...
    for (int i = ventana - 1; i > 0; i--) {
      lista.add(vehiclesNoSec.get(vehiclesNoSec.size() - i).getLow().get(opcion));
    }// fin for

    // Añade a la lista, los valores de la opción indicada, apartir de la lista de vehiculos (D)
    for (Item item : individuo) {
      lista.add(vehiclesSec.get(clases.get(item.getClase()).getSeqRank().get(0) - 1).getLow().get(opcion));
    }// fin for
